  ...
  for (const cdn of cdns) {
    try {
      const api = axios.create({
        headers: { "content-type": "application/json", origin: "https://yt.savetube.me", "user-agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36" },
        timeout: 25000,
      });
      ...
      const downloadRes = await api.post(`https://${cdn}/download`, { id: idMatch, downloadType: "audio", quality: "128", key: decrypted.key });
  ...
  if (subData.includes("<text")) {
    ...
    while ((match = regex.exec(subData)) !== null) {
      ...
      let text = match[3]
        ...
        .replace(/\n/g, " ")
        .trim();
      if (text) synced.push({ time: Math.round(startSec * 100) / 100, start: `${startSec.toFixed(2)}s`, text });
    }
    if (synced.length > 0) return synced;
  }

  const lines = subData.replace(/&amp;/g, "&").replace(/&#39;/g, "'").replace(/&quot;/g, '"').split("\n");
  for (let i = 0; i < lines.length; i++) {
    ...
    if (timeMatch) {
      ...
      if (text && !text.match(/^\d+$/) && !text.includes("-->")) {
        synced.push({ time: Math.round(startSec * 100) / 100, start: `${startSec.toFixed(2)}s`, end: `${timeToSeconds(endStr).toFixed(2)}s`, text });
      }
    }
  }
  return synced;
...
function formatAssemblyAIWords(words) {
  ...
  for (let i = 0; i < words.length; i++) {
    ...
    if (isLast || currentWords.length >= 7 || pause > 800 || /[.?!,]$/.test(w.text)) {
      ...
      lines.push({ time: Math.round(startSec * 100) / 100, start: `${startSec.toFixed(2)}s`, end: `${endSec.toFixed(2)}s`, text: currentWords.join(" "), confidence: w.confidence });
      currentWords = [];
    }
  }
  return lines;
...
export async function getTranscribe(urlOrVideoId) {
    ...
    if (capMatch) {
      ...
      if (cap?.baseUrl) {
        ...
        if (synced && synced.length > 0) {
          return { title, videoId, method: "subtitle", language: cap.languageCode || "en", text: synced.map((s) => s.text).join(" "), synced };
        }
      }
    }
  } catch (e) {}
  ...
  try {
    const uploadRes = await axios.post("https://api.assemblyai.com/v2/upload", fs.createReadStream(mp3Path), {
    ...
    for (let i = 0; i < 30; i++) {
      ...
      const poll = await axios.get(`https://api.assemblyai.com/v2/transcript/${transRes.data.id}`, { headers: { Authorization: ASSEMBLYAI_KEY }, timeout: 10000 });
