...
function findAllKeys(obj, keyToFind, results) {
  ...
  for (const key of Object.keys(obj)) {
    if (typeof obj[key] === "object") findAllKeys(obj[key], keyToFind, results);
  }
}
...
export async function GET(request) {
  ...
  if (!id) return NextResponse.json({ status: false, message: "Parameter id wajib diisi" }, { status: 400 });
  ...
  try {
    ...
    const data = await makeRequest(
      ...
      { context: { client: { clientName, clientVersion: isPlaylist ? "2.20240726.00.00" : "1.20240101.00.00", hl: "en", gl: "ID" } }, browseId }
    );
    ...
    if (isPlaylist) {
      ...
      if (sidebars.length > 0) {
        ...
        const thumbs =
          sidebars[0].thumbnailRenderer?.playlistVideoThumbnailRenderer?.thumbnail?.thumbnails ||
          sidebars[0].thumbnailRenderer?.playlistCustomThumbnailRenderer?.thumbnail?.thumbnails ||
      ...
      if (albumArtist.includes(" • ")) {
        albumArtist =
          albumArtist.split(" • ").find((x) => !["Album", "Single", "EP", "Playlist", "2023", "2024", "2022", "2021", "2020"].includes(x.trim())) ||
          albumArtist;
      }
      if (!albumArtist) {
      ...
      for (const item of items) {
        if (item.musicResponsiveListItemRenderer) {
          ...
          const videoId =
            i.playlistItemData?.videoId || i.flexColumns?.[0]?.musicResponsiveListItemFlexColumnRenderer?.text?.runs?.[0]?.navigationEndpoint?.watchEndpoint?.videoId || "";
          if (!videoId) continue;
          ...
          songs.push({ videoId, title: songTitle, artist, artistId, duration, thumbnails: thumb });
        }
      }
    }

    thumbnails = transformThumbs(thumbnails);
    if (thumbnails.length === 0 && songs.length > 0) thumbnails = songs[0].thumbnails || [];

    return NextResponse.json({
      status: songs.length > 0 || isPlaylist,
      creator: "ucamusic",
      result: { id, title, description, thumbnails, songs },
    });
  } catch (e) {
