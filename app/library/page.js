    ...
    <main className="px-4 pt-8">
      <div className="flex items-center justify-between mb-5">
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-3.5 py-2 rounded-full bg-white text-black text-xs font-semibold active:scale-95 transition-transform"
        >
      ...
      <div className="flex gap-2 mb-5">
        {tabs.map((t) => (
          <button
            ...
            className={tab === t.key ? "px-4 py-1.5 rounded-full text-xs font-semibold bg-white text-black" : "px-4 py-1.5 rounded-full text-xs font-medium text-white/60 glass"}
          >
            {t.label}
          </button>
        ))}
              ...
              {playlists.map((p) => (
                <div key={p.id} className="relative group">
                  ...
                  <button
                    ...
                    className="absolute top-2 right-2 p-1.5 rounded-full bg-black/60 text-white/70 opacity-0 group-hover:opacity-100 transition-opacity"
                  >
